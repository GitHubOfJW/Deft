  ...
  static validator(data = '',rules={
    ...
    regular:{ enable:false,regx:'',prompt:''}
      ...
      if(rules.required && (!data && typeof data === 'string' || validator.isEmpty(data))){
        const result = BaseController.handlerResponseData(0,`${name} 不能为空`)
        res.json(result);
        return true;
      }else if(rules.isEmail && !validator.isEmail(data)){
        const result = BaseController.handlerResponseData(0,`${name} 不合法`)
        res.json(result);
        return true;
        ...
        const result = BaseController.handlerResponseData(0,`${name} 必须为0或者1`)
        res.json(result);
        return true;
      }else if(rules.isInt && !validator.isInt(data)){
        const result = BaseController.handlerResponseData(0,`${name} 必须为整数`)
        res.json(result);
        return true;
      }else if(rules.min && rules.max && data && typeof data =='string' && data.trim().length < rules.min && data.trim().length > rules.max){
        const result = BaseController.handlerResponseData(0,`${name} 必须为 ${rules.min}到${rules.max}个字符`)
        res.json(result);
        return true;
      }else if(rules.min && data && typeof data =='string' && data.trim().length < rules.min){
        const result = BaseController.handlerResponseData(0,`${name} 不得少于${rules.min}个字符`)
        res.json(result);
        return true;
      }else if(rules.max && data && typeof data =='string' && data.trim().length > rules.max){
        const result = BaseController.handlerResponseData(0,`${name} 不得多于${rules.min}个字符`)
        res.json(result);
        return true;
      }else if(rules.regular && rules.regular.enable && data && !rules.regular.regx.test(data)){
        const result = BaseController.handlerResponseData(0,rules.regular.prompt)
        res.json(result);
        return true;
      }
